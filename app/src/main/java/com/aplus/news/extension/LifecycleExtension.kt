...
fun <T> Fragment.collectViewLifecycleFlow(flow: Flow<T>, collect : suspend (T) -> Unit) {
...
fun <T> Fragment.collectLatestViewLifecycleFlow(flow: Flow<T>, collect : suspend (T) -> Unit) {
    viewLifecycleOwner.lifecycleScope.launch {
        repeatOnLifecycle(Lifecycle.State.STARTED) {
            flow.collectLatest(collect)
        }
